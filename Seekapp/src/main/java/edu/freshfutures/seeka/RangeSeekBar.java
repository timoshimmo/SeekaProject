...
public class RangeSeekBar <T extends Number> extends ImageView {
    ...
    private static final int DEFAULT_TEXT_SIZE_IN_DP = 20;
    private static final int DEFAULT_TEXT_DISTANCE_TO_BUTTON_IN_DP = 10;
    ...
    public void init(Context context, AttributeSet attrs) {
        float barHeight;
        int thumbNormal = R.mipmap.seekbar_thumb;
        int thumbPressed = R.mipmap.seekbar_thumb;
        int thumbDisabled = R.mipmap.seekbar_thumb;
        ...
        if (attrs == null) {
            ...
            mActiveColor = context.getResources().getColor(R.color.pblue_color);
            mDefaultColor = context.getResources().getColor(R.color.super_light_gray);
            mAlwaysActive = false;
            ...
            try {
                ...
                mShowLabels2 = a.getBoolean(R.styleable.RangeSeekBar_showLabels2, true);
                ...
                Drawable normalDrawable = context.getResources().getDrawable(R.mipmap.seekbar_thumb);
                if (normalDrawable != null) {
                    thumbImage = BitmapUtil.drawableToBitmap(normalDrawable);
                }
                Drawable disabledDrawable = context.getResources().getDrawable(R.mipmap.seekbar_thumb);
                if (disabledDrawable != null) {
                    thumbDisabledImage = BitmapUtil.drawableToBitmap(disabledDrawable);
                }
                Drawable pressedDrawable = context.getResources().getDrawable(R.mipmap.seekbar_thumb);
        ...
        mTextSize = PixelUtil.pxToDp(context, DEFAULT_TEXT_SIZE_IN_DP);
        mDistanceToTop = PixelUtil.dpToPx(context, DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP);
        mTextOffset = !mShowTextAboveThumbs ? 0 : this.mTextSize + PixelUtil.pxToDp(context,
    ...
    private void setValuePrimAndNumberType() {
        absoluteMinValuePrim = absoluteMinValue.intValue();
        absoluteMaxValuePrim = absoluteMaxValue.intValue();
    ...
    protected synchronized void onDraw(@NonNull Canvas canvas) {
        ...
        if (mShowLabels) {
            ...
            float minMaxHeight = mTextOffset + mThumbHalfHeight + mTextSize + 30;

            canvas.drawText(minLabel, mInternalPad , minMaxHeight, paint);
            canvas.drawText(maxLabel, getWidth() - minMaxLabelSize - mInternalPad, minMaxHeight, paint);
        }
        ...
        if (mShowLabels2) {
            ...
            // draw min and max labels
            String minLabel = getContext().getString(R.string.demo_min_label);
            String maxLabel = getContext().getString(R.string.demo_max_label2);
            minMaxLabelSize = Math.max(paint.measureText(minLabel), paint.measureText(maxLabel));
            float minMaxHeight = mTextOffset + mThumbHalfHeight + mTextSize + 30;
